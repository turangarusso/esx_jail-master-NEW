...
--[[ RegisterCommand("prigione", function(_src, arg)

	print("eseguo comando")
	local xPlayer = ESX.GetPlayerFromId(_src)
	local pr = TriggerClientEvent('esx_jail:arresta', _src)
    local xId = GetPlayerServerId(pr)
	print(xId)
	--print(xPlayer.getJob().name)

	if xPlayer.getJob().name == "police" then 
		TriggerEvent('esx_jail:sendToJail', xId, arg * 60)
	end
end)
 ]]
ESX.RegisterCommand('jail1', 'admin', function(xPlayer, args, showError)
